...
public class LoginController {
    ...
    @PostMapping("save")
    public String saveStore(@ModelAttribute("user") @Valid User user, BindingResult bindingResult, Model model) {
        if (bindingResult.hasErrors()) {
            // Validation errors occurred, return to the registration form with error messages
            return "new_user";
        }

        // Validation successful, proceed with saving the user
        userService.save(user);
        String register_success = "<h5> Registration Successful! </h5>";
